...
===============================================================
All UI layout lives in `signature_viewer.ui` (editable in Qt Designer):
a QMainWindow with two QDockWidgets — "PDF" on top, a read-only
"Signatures & QCStatements" pane on the bottom — plus the toolbar and its
actions. This module loads that .ui at runtime and supplies only logic:
PDF page rendering, the Trusted-List download + cryptographic validation
(on a background thread), and signal/slot wiring.

The signature/QCStatement report is produced by the same classes used on the
command line (SignedPdf, EuTrustedListClient, ValidationContextBuilder,
QcStatementParser).

Note: pyhanko logs path-building failures to stderr for untrusted signatures;
those are intentionally left visible for now.
...
# ══════════════════════════════════════════════════════════════════════════════
# Main window: loads the .ui, then wires logic to the widgets it created
# ══════════════════════════════════════════════════════════════════════════════
...
class MainWindow(QMainWindow):
    def __init__(self, pdf_path: str, opts: dict, autostart: bool = True):
        ...
        self.resizeDocks([self.pdfDock, self.infoDock], [680, 340], Qt.Vertical)
        ...
        self.statusBar().showMessage(f"{self._renderer.page_count} page(s)")
