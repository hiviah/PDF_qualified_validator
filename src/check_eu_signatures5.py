...
# QcType OIDs below. The esign/eseal/web OIDs are *values inside* this sequence,
...
# QcCompliance – this cert is qualified (statementId, no statementInfo)
OID_QC_COMPLIANCE       = "0.4.0.1862.1.1"
# QcSSCD – private key is in SSCD (secure device) (statementId, no statementInfo)
...
def parse_qc_statements(cert: x509.Certificate) -> dict:
    """
    Parse the id-etsi-ext-qcStatements extension (OID 1.3.6.1.5.5.7.1.3) per
    ETSI EN 319 412-5.
    ...
    Most statements (QcCompliance 0.4.0.1862.1.1, QcSSCD 0.4.0.1862.1.4) are
    identified by statementId alone. The QcType statement is different:
    ...
    sequence — they are NOT statement IDs. So to detect "natural person" we must
    open the QcType statement's statementInfo and walk its OID list.

    Returns a dict with has_qc_statements, qc_compliance, qc_sscd, qct_esign,
    qct_eseal, qct_web, plus raw_oids (statement IDs) and qc_type_oids (the
    values found inside the QcType sequence).
    """
    result = {
        ...
        "qc_compliance": False,
        "qc_sscd": False,
        "raw_oids": [],       # statement IDs (one per QCStatement)
        "qc_type_oids": [],   # OIDs found inside the QcType sequence
    }
    ...
    try:
        ...
        raw_value = ext.value.value  # DER of QCStatements (SEQUENCE OF QCStatement)
        statements = asn1core.SequenceOf.load(raw_value)

        for stmt in statements:
            try:
                # stmt.contents = body of the QCStatement SEQUENCE:
                #   statementId (OID TLV) [+ statementInfo TLV]
                body = stmt.contents

                stmt_id_obj = asn1core.ObjectIdentifier.load(body)  # reads first TLV
                stmt_id = stmt_id_obj.dotted
                result["raw_oids"].append(stmt_id)

                # Bytes after the statementId OID = statementInfo (may be empty)
                consumed = len(stmt_id_obj.dump())
                info_bytes = body[consumed:]
                ...
                if stmt_id == OID_QC_COMPLIANCE:
                    ...
                    for t in qc_types:
                        try:
                            ...
                            # Fallback: child may already expose contents as an OID body
                            continue
                        result["qc_type_oids"].append(type_oid)
                ...
                # Skip a malformed individual statement, keep parsing the rest
                pass
    ...
    with open(pdf_path, "rb") as f:
        ...
        for emb in embedded:
            ...
            if qc["has_qc_statements"]:
                ...
                if qc["raw_oids"]:
                    print(f"    Statement IDs : {', '.join(qc['raw_oids'])}")
                if qc["qc_type_oids"]:
